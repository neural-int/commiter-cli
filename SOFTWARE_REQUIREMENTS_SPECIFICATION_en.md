...
Structural evidence itself must not include relationship labels or grouping recommendations such as change intent, feature, source/test, docs/source, same logical change, `test_for`, `related_to`, or `should_group`.

A separate internal API may derive relations between file IDs within one Git snapshot from Git renames, matching source/test paths, direct imports between changed files, identifier names observed at changed locations, shared directories, and known manifest/lockfile pairs. File IDs are not persistent across runs. A relation carries a kind, hard/soft classification, and machine-readable evidence. A rename is a hard self-relation for the single file ID that holds both paths; all other initial relations are soft. Identifier-name matches do not mean symbols were resolved. Ambiguous, unresolved, and unsupported extraction attempts are retained as diagnostic data separately from relations and are not relation edges. Shared-directory proximity is a grouped auxiliary hint and is not expanded into pairwise edges. The planning input may include candidate components, directed edges with evidence, auxiliary hints, and aggregate counts for ambiguous, unresolved, and unsupported observations within the existing context budget, while retaining every changed file ID. If the budget is exceeded, relation extraction or graph construction fails, or edges, hints, and observations are all empty, the complete relation context is omitted and the existing planning input is used. Further effectiveness thresholds are determined from measurements. If the initial prompt with relation context exceeds the budget, omit the graph before compressing existing file or hunk evidence. Keep the omission reason and, when available, bounded counts of ambiguous, unresolved, and unsupported observations in the planning input; do not include error text. Candidate components do not determine final commit groups; the LLM determines the purpose and grouping of every file ID. The LLM output JSON schema does not change.
