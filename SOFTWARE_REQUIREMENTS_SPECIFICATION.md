...
`scope` は必須の単一行文字列とし、空文字列は許可しません。また、`feat`、`fix`、`docs`、`style`、`refactor`、`perf`、`test`、`build`、`ci`、`chore`、`revert` のいずれかの Conventional Commit type 名をそのまま scope に使用してはなりません。該当する計画は生成後の決定的検証で無効として検出し、自動修復の対象にします。
...
不正な type、空の scope、Conventional Commit type 名と同じ scope（`feat(feat)`、`test(test)`、`fix(perf)` など）、複数行の summary、body の存在、file ID の欠落・重複・範囲外割り当て、複数コミットへの同一ファイルの割り当てを検出することを確認します。type 名 scope を含む候補は1回の自動修復に送られ、修復後も違反が残る場合は Git の状態を変更せずに停止することを確認します。
