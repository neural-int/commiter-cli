...
`llm.context` の許容値は `"auto"` および実装が対応する固定コンテキスト段階です。

`llm.max_context_tokens` は実装が対応するコンテキスト段階から設定し、`"auto"` の場合に自動選択できる最大値を指定します。対応値と既定値は設定検証および配布時のテストで固定します。
...
構造エビデンス自体には、変更目的、機能（feature）、実装とテスト（source / test）、ドキュメントとソースコード（docs / source）、同一の論理的変更、`test_for`、`related_to`、`should_group` などの関係性ラベルやグルーピングの推奨を含めてはなりません。

別の内部 API では、Git rename、対応する source / test のパス、変更ファイル間の直接 import、変更箇所に現れる識別子名、同一ディレクトリ、既知の manifest / lockfile の組から、同一 Git snapshot 内の file ID 間 relation を抽出できます。file ID は実行をまたぐ永続 ID ではありません。relation は kind、hard / soft、機械可読な根拠を持ちます。rename は変更前後を保持する単一 file ID の hard relation とし、それ以外の初期 relation は soft とします。識別子名の一致はシンボル解決を意味しません。曖昧・未解決・未対応の抽出試行は relation と分けた診断データとして保持し、relation edge として扱いません。同一ディレクトリの近接情報は file ID をまとめた補助 hint とし、全 file pair の edge に展開しません。planning 入力には全変更 file ID を維持したまま、候補 component、有向 edge と根拠、補助 hint、曖昧・未解決・未対応の件数集計を、既存の context 予算内で任意に含められます。予算を超える場合、relation の抽出・graph 構築に失敗した場合、または edge・hint・observation がすべて空の場合は relation context 全体を省略し、従来の planning 入力を使います。情報量ゼロ以外の有効性閾値は計測結果から決めます。relation context の初回入力が予算を超えた時点で省略し、そのために従来の file / hunk 根拠を先に圧縮しません。省略理由と、取得できた場合に限り曖昧・未解決・未対応の観測件数集計を固定された項目で planning 入力に残し、エラー本文は渡しません。候補 component は最終 commit group を決めず、すべての file ID の目的と grouping は LLM が判断します。LLM 出力 JSON schema は変更しません。
...
`llm.context = "auto"` の場合は、通常予算を上限として最小のコンテキスト段階を選択します。通常予算内で許容可能な最弱の chunk 圧縮 profile を適用しても収まらず、かつ `llm.max_context_tokens` がより大きな段階を許可する場合に限り、同じ profile の最終プロンプトを拡張後の予算で再計測します。それでも収まらない場合にのみ、拡張後の予算でより強い profile へ進みます。`llm.context` が固定値の場合は、その指定段階を超えて自動昇格してはなりません。
...
CLI は chunk 圧縮 profile またはコンテキスト予算を遷移するたびに最終プロンプトを再生成して上限見積もりを再計算し、対応リソース範囲内で収まる、情報損失が最も少ない組み合わせで停止します。profile を強めても選択行、最終プロンプトの UTF-8 バイト数、および上限見積もりは増加してはなりません。
...
不正な JSON、JSON スキーマ違反、対象 file ID の欠落・重複・範囲外割り当て、type 名をそのまま scope に使用した計画、または SR-010 による機密値の一致を検出した場合は、自動修復（repair）を1回だけ実行します。複数の違反を同時に検出した場合も、それらを1つの修復リクエストにまとめ、修復の試行回数を追加してはなりません。
...
実装変更と、それに直接対応するテスト変更は、分離する具体的な理由がない限り同じコミット候補に含めることを優先しなければなりません。LLM は差分の意味から分離理由を判断し、機械処理側はこの判断を後から書き換えてはなりません。
...
対話的な通常実行では、CLI はリポジトリの内容を含まない HTTP GET を固定された公式 GitHub Releases metadata エンドポイントへ最大24時間に1回だけ送信し、現在のバージョンより新しい安定版がある場合に通知しなければなりません。更新確認の試行時刻は HTTP 要求前に state ディレクトリへ保存し、保存に失敗した場合は HTTP 要求を行わずエラーを返さなければなりません。更新確認のネットワークまたは state 保存のエラーは通常処理を妨げず、JSON 出力および CI 環境では更新確認を行ってはなりません。
...
`scope` は必須の単一行文字列とし、空文字列は許可しません。また、前後の空白を除去したうえで大文字小文字を区別せず、`feat`、`fix`、`docs`、`style`、`refactor`、`perf`、`test`、`build`、`ci`、`chore`、`revert` のいずれかの Conventional Commit type 名と一致する scope を使用してはなりません。該当する計画は生成後の決定的検証で無効として検出し、自動修復の対象にします。

`breaking` は boolean とし、`true` は公開 API、CLI、設定形式、または保存形式の非互換変更を差分から説明できる場合だけに指定します。通常の UI 変更や内部リファクタリングだけでは `breaking=true` にしてはなりません。`true` の場合のみ件名（subject）の scope の直後に `!` を付与します。

`perf` は性能改善が変更目的であり、差分または対応する検証結果から性能改善の根拠を確認できる場合だけに指定します。性能根拠のない JSX 整理や責務移動などの構造変更は `refactor` とします。
...
入力サイズが通常予算を超える場合であっても、設定された最大予算までの適応的拡張と階層要約を用いて、対象ファイルの完全な集合を保持したまま計画生成を継続しなければなりません。
...
実装が対応する各コンテキスト段階に収まるケースと最大上限を超えるケースを用意し、最終プロンプトの UTF-8 バイト数、チャットテンプレート用の 256 トークン、および `max(1024, 48 × 対象ファイル数)` の出力予約トークンに基づいて、許可された最小のコンテキスト段階が選択されることを確認します。

`llm.context = "auto"` では、通常予算と最弱 profile、拡張予算と同じ profile、拡張予算とより強い profile の順に評価され、固定コンテキスト設定では指定段階を超えて自動昇格しないことを確認します。各遷移で最終プロンプトが再生成・再計測され、同一入力と profile の出力が選択経路に依存せず決定論的であることを確認します。最大予算と最強 profile でも上限を超える場合は、LLM を呼び出すことなく Git を変更せずに停止することを確認します。
...
実装コード、対応するテスト、ドキュメント、依存関係の変更、機械的変更を含む差分において、LLM が意味・目的に基づいてグルーピングを行い、実装変更と直接対応するテスト変更を分離する具体的な理由がない場合は同じコミット候補に含め、機械処理側が仕様上有効なグルーピングを書き換えないことを確認します。file ID の欠落・重複・範囲外割り当てが検出されること、ならびに同一ファイルの hunk 分割が行われないことも確認します。opaque file についてのみ、メタデータをグルーピングの補助的な判断材料として使用できることを確認します。
...
不正な type、空の scope、前後の空白や大文字小文字の違いを含む Conventional Commit type 名と同じ scope（`feat(feat)`、`test(test)`、`fix(perf)`、`fix(PERF)` など）、複数行の summary、body の存在、file ID の欠落・重複・範囲外割り当て、複数コミットへの同一ファイルの割り当てを検出することを確認します。type 名 scope を含む候補は1回の自動修復に送られ、修復後も違反が残る場合は Git の状態を変更せずに停止することを確認します。
...
対話的な通常実行でリポジトリの内容を送信せず、更新確認の試行時刻が HTTP 要求前に保存され、保存できない場合は HTTP 要求を行わずエラーを返すことを確認します。ネットワークまたは state 保存のエラーが通常処理を妨げないこと、JSON 出力および CI 環境で更新確認を行わないことも確認します。
